...
public class TreeEvaluator implements FitnessEvaluator<Node>
{
    ...
    private final transient ThreadLocal<Renderer<Node, BufferedImage>> threadLocalRenderer
    = new ThreadLocal<Renderer<Node, BufferedImage>>();
     ...
     */
    public double getFitness(Node candidates, List<? extends Node> population)
    {
    	/*
    	Renderer<Node, BufferedImage> renderer = threadLocalRenderer.get();
    	if (renderer == null)
        {
        ...
        */
        ...
        int counter = 0;
        Node candidate = null;
        if (candidates instanceof MultipleNode) {
        	for (Node n: ((MultipleNode)candidates).nodes) {
        		candidate = n;

    	        for (Map.Entry<double[], double[]> entry : data.entrySet())
    	        {
    	            double actualValue = candidate.evaluate(entry.getKey());
    	            if (actualValue == entry.getValue()[counter]) {
    	            	error++;
    	            }    	         
    	            //double diff = actualValue - entry.getValue()[counter];
    	            //error += (diff * diff);
