 ...
 * @author Daniel Dyer
 */
public abstract class UnitaryNode implements Node
{
     ...
     */
    protected UnitaryNode(Node left, Node right, char symbol)
    {
        this.left = left;
        //this.right = right;
        this.symbol = symbol;
    ...
    public Node replaceNode(int index, Node newNode)
    {
        ...
        int leftNodes = left.countNodes();
        return newInstance(left.replaceNode(index - 1, newNode), null);
        /*
        if (index <= leftNodes)
        {
    ...
    public Node mutate(Random rng, Probability mutationProbability, TreeFactory treeFactory)
    {
        ...
        else
        {
            ...
            if (newLeft != left)
            {
                return newInstance(newLeft, null);
            }
            else
            {
    ...
    {
        Constructor<? extends UnitaryNode> constructor = ReflectionUtils.findKnownConstructor(this.getClass(),
                                                                                             Node.class,
                                                                                             Node.class);
