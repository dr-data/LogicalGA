...
{
    private final ArrayTreeFactory<Node> treeFactory;
     ...
     */
    public TreeMutation(ArrayTreeFactory treeFactory,
    ...
    public List<Node> apply(List<Node> selectedCandidates, Random rng)
    {
        ...
        {
        	if (tree instanceof MultipleNode) {
        		TreeFactory f = treeFactory.getFactories().get(0);
        		for (Node t : ((MultipleNode)tree).nodes) {
        			mutatedPopulation.add(t.mutate(rng, mutationProbability, f));
        		}
        	}
        	else {
        		TreeFactory f = treeFactory.getFactories().get(0);
        		mutatedPopulation.add(tree.mutate(rng, mutationProbability, f));        		
        	}
        }
